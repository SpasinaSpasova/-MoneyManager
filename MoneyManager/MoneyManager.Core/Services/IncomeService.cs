...
namespace MoneyManager.Core.Services
{
    public class IncomeService : IIncomeService
    {
        ...
        {
            var product = await repo.GetByIdAsync<Income>(id);
            if (product != null)
            {
                product.IsActive = false;

                var account = await repo.GetByIdAsync<Account>(product.AccountId);
                account.Amount -= product.Amount;
        ...
        public async Task Upload(Guid id, IFormFileCollection files)
        {
